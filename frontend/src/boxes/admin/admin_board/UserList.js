...
const UserList = () => {
  ...
  const handleSearch = (searchWord) => {
    ...
    if (selectedOption) {
      axios
        .get(
          `http://3.34.251.151:8080/admins/searchUserList/${selectedOption}/${searchWord}/${currentPage}`
        )
        .then((response) => {
  ...
  const handleUserDetail = (userId) => {
    axios
      .get(`http://3.34.251.151:8080/users/${userId}`)
      .then((response) => {
  ...
  const getUserList = () => {
    axios
      .get(`http://3.34.251.151:8080/admins/userList/${currentPage}`)
      .then((response) => {
        setTotalUsers(response.data.totalUsers);
        setUserList(response.data.users);
        setTotalPages(response.data.totalPages);
