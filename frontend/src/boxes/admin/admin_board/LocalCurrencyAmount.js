...
const LocalCurrencyAmount = () => {
  ...
  useEffect(() => {
    axios
      .get(`http://3.34.251.151:8080/admins/sales/list/${currentPage}`)
      .then((res) => {
  ...
  useEffect(() => {
    axios
      .get(`http://3.34.251.151:8080/admins/currency/month/total`)
      .then((res) => {
        const dataKey = [];
        const dataValue = [];
  ...
  useEffect(() => {
    if (currencyName === "") {
      axios
        .get(`http://3.34.251.151:8080/admins/voucher/sales-total`)
        .then((res) => {
          const datakeys = [];
          const datavalues = [];
          Object.entries(res.data).forEach(([key, value]) => {
  ...
  useEffect(() => {
    if (localSelect === "") {
      axios
        .get(`http://3.34.251.151:8080/admins/useChart/total`)
        .then((res) => {
          const dataKeys = [];
          const dataValues = [];
          Object.entries(res.data).forEach(([key, value]) => {
  ...
  const start_end_date = (e) => {
    ...
    if (currencyName === "") {
      ...
      alert(`같은 년도 이내로 선택해주세요.`);
    } else {
      axios
        .get(
          `http://3.34.251.151:8080/admins/voucher/name-list/${currencyName}/${startDate}/${endDate}`
        )
        .then((res) => {
  ...
  const use_start_end_date = (e) => {
    ...
    if (useStartDate > useEndDate) {
      ...
      axios
        .get(
          `http://3.34.251.151:8080/admins/useChart/test/${localSelect}/${useStartDate}/${useEndDate}`
        )
        .then((res) => {
          const useLocalKeys = [];
  ...
  const handleSearch = (searchWord) => {
    if (citySelect === "") {
      ...
      axios
        .get(`http://3.34.251.151:8080/admins/sales/search`, {
          params: {
        ...
        <div className="currencyTotal-div-byCurrencyName">
          <div id="graph-title">
            ...
            <h3 className="font-weight-bold">
              총 매출 건수 : {Intl.NumberFormat().format(salesCount)} 건 (
              2020.08 기준 )
            </h3>
