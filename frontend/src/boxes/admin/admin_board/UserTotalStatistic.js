...
const UserTotalStatistic = () => {
  ...
  const userAgeThunk = (localSelect) => {
    axios
      .get(`http://3.34.251.151:8080/admins/userTotal-chart/${localSelect}`)
      .then((res) => {
        const genderValues = [];
        const getnderKeys = [];
        const ageKeys = [];
  ...
  useEffect(() => {
    axios
      .get(`http://3.34.251.151:8080/admins/userTotal-chart/${"null"}`)
      .then((res) => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`http://3.34.251.151:8080/admins/chart/ratio-of-user-region`)
      .then((res) => {
        const values = [];
        const keys = [];
    ...
    <div>
      ...
      <div className="userLocal-Total">
        ...
        <h3 className="font-weight-bold">
          총 회원수: {Intl.NumberFormat().format(toTalUserCount)} 명
        </h3>
        <Bar data={chartData} />
      </div>
