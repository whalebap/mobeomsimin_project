...
const MyQeustion = () => {
  ...
  const handleQuestionSave = () => {
    ...
    axios
      .post(`http://3.34.251.151:8080/posts/questions/${id}`, data)
      .then(() => {
        alert("문의를 완료했습니다");
        refreshList(id);
        handleClose();
  ...
  const handleQuestionEdit = (questionId) => {
    ...
    axios
      .patch(`http://3.34.251.151:8080/posts/questions/${questionId}`, data)
      .then(() => {
        alert("수정을 완료했습니다.");
        refreshList(id);
        handleClose();
  ...
  const handleDelete = (questionId) => {
    axios
      .delete(`http://3.34.251.151:8080/posts/questions/${questionId}`)
      .then(() => {
        alert("삭제가 완료됐습니다.");
        refreshList(id);
      })
  ...
  const refreshList = (id) => {
    axios
      .get(`http://3.34.251.151:8080/posts/questions/${id}`)
      .then((response) => {
        const values = [];
        const keys = [];
