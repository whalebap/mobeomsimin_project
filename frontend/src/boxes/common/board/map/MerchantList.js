...
const MerchantList = () => {
  ...
  function setPageArr() {
    if (totalCount !== 0) {
      ...
      let tmpArr = [];
      let limitNum =
        pageCount !== 0 && pageNow + 1 === pageCount
          ? totalCount - pageNow * pageSize
          : pageSize;
  ...
  useEffect(() => {
    axios
      .get(
        `http://3.34.251.151:8080/stores/getSome/""/""/${
          blockNow * pageSize * blockSize
        }/${blockSize * pageSize}`
      )
      .then(({ data }) => {
  ...
  return (
    <div className="container" id={"merchan_list"}>
      ...
      <h3 style={{ float: "left" }}>
        &nbsp;&nbsp;총{" "}
        {totalCount.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}개
        가맹점&nbsp;
      </h3>
      ...
      <table className={"paging"}>
        <tr>
          ...
          {pageNumArr.map((num) => (
            <td className={"nums"} onClick={() => setPageNow(num)}>
              &nbsp;{num + 1}
            </td>
          ))}
          ...
          {existNext && (
            <td
              ...
              onClick={(e) => {
                e.preventDefault();
                setPageNow(parseInt(pageCount / blockSize) * blockSize);
              }}
            >
              &nbsp;{">>"}
            </td>
