...
const FindByMap = ({ isLogined }) => {
  ...
  };// 우리집 아이콘 클릭: 사용자 defaultAddr를 지도의 센터로
  ...
  };// get latitude, longitude from addr
  ...
  useEffect(() => {
    if (homePosit.lat) {
      axios
          .get(
              `http://3.34.251.151:8080/stores/fromAddr/${homePosit.lat}/${homePosit.lng}`
          )
          .then(({ data }) => {
            let temList = [];
            let bestList = [];
