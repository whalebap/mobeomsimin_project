...
export const StoreReport = ({ storeInfo, reportClose, modalClose }) => {
  const handleReport = () => {
    axios
      .post(`http://3.34.251.151:8080/reports/${storeInfo.id}`)
      .then(() => {
...
export const Star = ({ storeInfo, starClose, modalClose }) => {
  const addStore = () => {
    ...
    axios
      .post(`http://3.34.251.151:8080/favorites`, data)
      .then(() => {
        alert("즐겨찾기에 추가되었습니다.");
        modalClose();
      })
  ...
  return (
    <div className={"star_modal"}>
      <Modal autoFocus={true} show={true} onHide={starClose}>
        <Modal.Header closeButton>
          <Modal.Title>
            <img alt={"favIcon"} src={favStar} width={25} height={25} />
            &nbsp;즐겨찾기 추가
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
        ...
        <Modal.Footer>
          <button className={"addstarB"} onClick={addStore}>
            추가하기
          </button>
          <button className={"cancelB"} onClick={handleClose}>
            취소
          </button>
        </Modal.Footer>
      </Modal>
