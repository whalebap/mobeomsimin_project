...
const StoreChatbot = (props) => {
  ...
  useEffect(() => {
    if (searched !== "") {
      axios
        .get(
          `http://3.34.251.151:8080/stores/chatbotSearch/${searched}/${pageSize}`
        )
        .then(({ data }) => {
    ...
    <div>
      ...
      {storeList.length !== 0 ? (
        storeList.map((store, i) =>
          i % 10 ? (
            <div key={i}>
              <Link onClick={() => setStore(store)} to={"/storeDetail"}>
                {store.storeName}
              </Link>
            </div>
          ) : (
      ...
      {totalCount - pageSize > 0 && (
        <button onClick={() => setPageSize(pageSize + 10)}>더보기</button>
      )}
    </div>
  );
};
