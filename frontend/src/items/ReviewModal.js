...
const ReviewModal = ({
  ...
  const handleSearchStore = (e) => {
    ...
    if (storeNameModal) {
      axios
        .get(`http://3.34.251.151:8080/stores/findStore/${storeNameModal}`)
        .then((response) => {
          const values = [];
          const keys = [];
  ...
  const handleSave = (e) => {
    ...
    axios
      .post(`http://3.34.251.151:8080/posts/reviews/${selectedStoreId}`, data)
      .then(() => {
        alert("저장에 성공했습니다.");
        setReview("");
        onSubmit();
  ...
  const handleSaveModi = (e) => {
    ...
    console.log(data);
    axios
      .patch(`http://3.34.251.151:8080/posts/reviews/${reviewId}`, data)
      .then(() => {
        alert("수정이 완료됐습니다.");
        setReview("");
        onSubmit();
  ...
  useEffect(() => {
    if (reviewId) {
      axios
        .get(`http://3.34.251.151:8080/posts/reviews/detail/${reviewId}`)
        .then((response) => {
                ...
                <tr key={i}>
                  ...
                  <td>
                    <button
                      className="btn btn-outline-primary"
                      onClick={() => {
                        handlePickOneStore(storeIdArr[i], info);
                      }}
                    >
                      선택
                    </button>
